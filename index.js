...
const fs = require('fs');
const awsSdkCall = require('./aws-sdk-request')
...
function deployBeanstalkVersion(application, environmentName, versionLabel) {
    return awsSdkCall({ ApplicationName: application, EnvironmentName: environmentName, VersionLabel: versionLabel });
}
...
function main() {
    ...
    if (IS_GITHUB_ACTION) { //Running in GitHub Actions
        ...
        // Calling this action after aws-actions/configure-aws-credentials@v1
        // will have these variables in environment
        ...
        awsApiRequest.region = strip(process.env.AWS_REGION);
